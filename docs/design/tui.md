...
- **ID = blake3(content) 的 hash**，显示为 `S@a3f1`（短 hash）
- 相同环境 = 相同 hash = 自动去重
...
规则：
  ...
  - `:run cd ...` / `:run env set ...` 生成 **job-local end_scope**，不会自动移动当前 session cursor
  - 顶层 `:cd` / `:env set ...` 只修改当前 logical session 的 cursor；使用相同 session id 重连会恢复它
...
session cursor ≈ 可移动的 checked-out ref
```
