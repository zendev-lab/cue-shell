...
///
/// Initialises a fallback root scope from the current process environment. New
/// client sessions insert their own handshake-provided root scopes.
pub(super) async fn spawn(
    mut rx: mpsc::Receiver<ScopeStoreMsg>,
    conn: Connection,
    _sys: super::ActorSystem,
) -> Result<()> {
    let db = storage::shared_connection(conn);
    let mut cache = create_and_persist_root_scope(&db).await?;

    tokio::spawn(async move {
        info!(
            root_count = cache.len(),
            "scope_store: started immutable scope store"
        );
            ...
            match msg {
                ScopeStoreMsg::Insert { scope, reply } => {
                    ...
                    match result {
                        ...
                        Err(error) => {
                            error!("scope_store: persist inserted scope failed: {error}");
                            let _ = reply.send(Err(anyhow::anyhow!(
                                "persist inserted scope {hash}: {error}"
                            )));
                        }
                    }
                }
                ...
                ScopeStoreMsg::ListScopes { reply } => {
                    ...
                    scope_infos.sort_by(|a, b| a.hash.cmp(&b.hash));
                    let _ = reply.send(Ok(scope_infos));
                }
            }
        }
...
mod tests {
    ...
    #[tokio::test]
    async fn insert_scope_roundtrips_handshake_root() {
        ...
        scope_tx
            ...
            .await
            .expect("insert scope");
        let hash = tokio::time::timeout(std::time::Duration::from_secs(1), insert_rx)
            .await
            .expect("insert reply")
            .expect("insert sender")
            .expect("insert result");
        assert_eq!(hash, scope.hash);
        ...
        scope_tx
            ...
            .await
            .expect("get scope");
        let restored = tokio::time::timeout(std::time::Duration::from_secs(1), get_rx)
            .await
            .expect("get reply")
            .expect("get sender")
            .expect("get result")
    ...
    async fn list_scopes_reads_persisted_scopes_not_only_cache() {
        ...
        let scopes = tokio::time::timeout(std::time::Duration::from_secs(1), reply_rx)
            .await
